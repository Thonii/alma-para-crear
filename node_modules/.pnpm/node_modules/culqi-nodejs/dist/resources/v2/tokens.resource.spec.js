"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const culqi_client_1 = require("../../client/culqi-client");
const http_client_1 = require("../../client/http-client");
let sdk;
beforeEach(() => {
    sdk = culqi_client_1.CulqiClient.init({
        publicKey: 'pk_test_x',
        secretKey: 'sk_test_x',
        apiVersion: '2',
    });
});
afterEach(() => jest.restoreAllMocks());
describe('Tokens - Unit Test (v2)', () => {
    it('should create a card token', async () => {
        const postSpy = jest.spyOn(http_client_1.HttpClient.prototype, 'post').mockResolvedValue({ id: 'tkn_123' });
        const payload = {
            card_number: '4111111111111111',
            cvv: '123',
            expiration_month: '09',
            expiration_year: '2025',
            email: 'janedoe@domain.com',
        };
        const res = await sdk.tokens.create(payload);
        expect(postSpy).toHaveBeenCalledWith('/v2/tokens', {
            data: payload,
            pub: true,
        });
        expect(res).toEqual({ id: 'tkn_123' });
        postSpy.mockRestore();
    });
    it('should get a list of tokens with given query params', async () => {
        const postSpy = jest.spyOn(http_client_1.HttpClient.prototype, 'get').mockResolvedValue({ data: [] });
        const query = { limit: 10, country_code: 'US' };
        await sdk.tokens.findBy(query);
        expect(postSpy).toHaveBeenCalledWith('/v2/tokens', {
            params: query,
            pub: false,
        });
    });
    it('should get a single token by its id', async () => {
        const postSpy = jest.spyOn(http_client_1.HttpClient.prototype, 'get').mockResolvedValue({ id: 'tkn_123' });
        const res = await sdk.tokens.find('tkn_123');
        expect(postSpy).toHaveBeenCalledWith('/v2/tokens/tkn_123', { pub: false });
        expect(res).toEqual({ id: 'tkn_123' });
        postSpy.mockRestore();
    });
    it('should update a token', async () => {
        const postSpy = jest.spyOn(http_client_1.HttpClient.prototype, 'patch').mockResolvedValue({ id: 'tkn_123' });
        const payload = { metadata: { foo: 'bar' } };
        await sdk.tokens.update('tkn_123', payload);
        expect(postSpy).toHaveBeenCalledWith('/v2/tokens/tkn_123', {
            data: payload,
            pub: false,
        });
    });
    it('should create a yape token', async () => {
        const postSpy = jest.spyOn(http_client_1.HttpClient.prototype, 'post').mockResolvedValue({ id: 'ype_123' });
        const payload = {
            otp: '946627',
            number_phone: '951123456',
            amount: 500,
        };
        const res = await sdk.tokens.createYape(payload);
        expect(postSpy).toHaveBeenCalledWith('/v2/tokens/yape', {
            data: payload,
            pub: true,
        });
        expect(res).toEqual({ id: 'ype_123' });
        postSpy.mockRestore();
    });
});
//# sourceMappingURL=tokens.resource.spec.js.map