"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const culqi_client_1 = require("../../client/culqi-client");
const http_client_1 = require("../../client/http-client");
let sdk;
beforeEach(() => {
    sdk = culqi_client_1.CulqiClient.init({
        publicKey: 'pk_test_x',
        secretKey: 'sk_test_x',
        apiVersion: '2',
    });
});
afterEach(() => jest.restoreAllMocks());
describe('Plans – Unit Test (v2)', () => {
    it('should create a plan', async () => {
        const postSpy = jest.spyOn(http_client_1.HttpClient.prototype, 'post').mockResolvedValue({ id: 'pln_123' });
        const payload = {
            name: 'Plan Business Plus.',
            short_name: 'Business Plus',
            description: 'Lorem ipsum...',
            amount: 5000,
            currency: 'PEN',
            interval_unit_time: 1,
            interval_count: 1,
            initial_cycles: {
                count: 0,
                has_initial_charge: false,
                amount: 0,
                interval_unit_time: 1,
            },
        };
        const res = await sdk.plans.create(payload);
        expect(postSpy).toHaveBeenCalledWith('/v2/recurrent/plans/create', {
            data: payload,
            pub: false,
        });
        expect(res).toEqual({ id: 'pln_123' });
        postSpy.mockRestore();
    });
    it('should get a list of plans with given query params', async () => {
        const postSpy = jest.spyOn(http_client_1.HttpClient.prototype, 'get').mockResolvedValue({ data: [] });
        const query = { status: 1, creation_date_to: 1671720949000 };
        await sdk.plans.findBy(query);
        expect(postSpy).toHaveBeenCalledWith('/v2/recurrent/plans', {
            params: query,
            pub: false,
        });
    });
    it('should get a single plan by its id', async () => {
        const postSpy = jest.spyOn(http_client_1.HttpClient.prototype, 'get').mockResolvedValue({ id: 'pln_123' });
        const res = await sdk.plans.find('pln_123');
        expect(postSpy).toHaveBeenCalledWith('/v2/recurrent/plans/pln_123', { pub: false });
        expect(res).toEqual({ id: 'pln_123' });
        postSpy.mockRestore();
    });
    it('should update a plan', async () => {
        const postSpy = jest.spyOn(http_client_1.HttpClient.prototype, 'patch').mockResolvedValue({ id: 'pln_123' });
        const payload = {
            name: 'Plan Enterprise',
            short_name: 'Enterprise',
            status: 1,
        };
        await sdk.plans.update('pln_123', payload);
        expect(postSpy).toHaveBeenCalledWith('/v2/recurrent/plans/pln_123', {
            data: payload,
            pub: false,
        });
    });
    it('should delete a plan', async () => {
        const postSpy = jest
            .spyOn(http_client_1.HttpClient.prototype, 'del')
            .mockResolvedValue({ id: 'pln_123', deleted: true });
        const res = await sdk.plans.remove('pln_123');
        expect(postSpy).toHaveBeenCalledWith('/v2/recurrent/plans/pln_123', { pub: false });
        expect(res).toEqual({ id: 'pln_123', deleted: true });
        postSpy.mockRestore();
    });
});
//# sourceMappingURL=plans.resource.spec.js.map